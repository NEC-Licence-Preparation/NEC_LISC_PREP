...
import BookmarkTestRunner from "@/components/BookmarkTestRunner";
...
export default async function TestPage({
  ...
  searchParams:
    | Promise<{ subject?: string; mode?: string; count?: string; bookmarks?: string }>
    | { subject?: string; mode?: string; count?: string; bookmarks?: string };
}) {
  ...
  const count = resolvedParams?.count
    ...
    : undefined;
  const bookmarks = resolvedParams?.bookmarks;
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4 sm:space-y-5">
        {bookmarks && (
          <h1 className="text-xl sm:text-2xl font-semibold text-[#424874]">
            {bookmarks === "all" ? "All Bookmarked Questions" : "Bookmarked Question"}
          </h1>
        )}
        {!bookmarks && mode === "practice" && (
          <h1 className="text-xl sm:text-2xl font-semibold text-[#424874]">
            Practice Test
          </h1>
        )}
        {!bookmarks && mode === "daily10" && (
          <h1 className="text-xl sm:text-2xl font-semibold text-[#424874]">
            Daily 10-Question Test
          </h1>
        )}
        {!bookmarks && mode === "daily100" && (
          <h1 className="text-xl sm:text-2xl font-semibold text-[#424874]">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)] gap-4 lg:gap-6 items-start">
          {bookmarks && <BookmarkTestRunner bookmarkIds={bookmarks} />}
          {!bookmarks && mode === "practice" && (
            <TestRunner subject={subject} preselectedCount={count} />
          )}
          {!bookmarks && mode === "daily10" && <DailyTestRunner setSize="10" />}
          {!bookmarks && mode === "daily100" && <DailyTestRunner setSize="100" />}
        </div>
      </main>
    </div>
  );
